...
exports.getShipLengthFromType = function (type) {
    switch (type) {
        case schemas_1.ShipType.Battleship:
            return 4;
        case schemas_1.ShipType.Cruiser:
            return 3;
        case schemas_1.ShipType.Destroyer:
            return 2;
        case schemas_1.ShipType.Submarine:
            return 1;
        default:
...
exports.didShotHit = function (latitude, longitude, ships) {
    for (var _i = 0, ships_1 = ships; _i < ships_1.length; _i++) {
        var ship = ships_1[_i];
        if (exports.doesPointIntersectWithShip(Number(latitude), Number(longitude), ship)) {
            return true;
        }
    }
    return false;
};
exports.doesPointIntersectWithShip = function (latitude, longitude, ship, bufferZone) {
    if (bufferZone === void 0) { bufferZone = 0; }
    ...
    if (ship.orientation === 'vertical') {
        ...
        for (var long = ship.longitude; long < ship.longitude + shipLength; long++) {
            if (lat <= latitude + bufferZone &&
                lat >= latitude - bufferZone &&
                long <= longitude + bufferZone &&
                long >= longitude - bufferZone) {
